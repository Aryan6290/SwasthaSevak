    ...
    async initializeBedCount(req: Request, res: Response) {
        try {
            ...
            const insert = await this.db
                ...
                .insertOne({ hospitalId: new ObjectId(hospitalId), bedCount });
            if (insert.insertedCount > 0) {
                res.status(200).send({ status: true, message: "beds created" });
            } else {
                res
                    .status(400)
                    .send({ status: false, message: "data cant be inserted" });
            }
        } catch (err) {
            console.log(err);
            res.status(500).send({ status: false, message: "Error in Backend" });
    ...
    async updateBedCount(req: Request, res: Response) {
        try {
            ...
            const update = await this.db
                .collection(COLLECTIONS.BEDS)
                .updateOne({ hospitalId }, { $set: { bedCount: newCount } });
    ...
    async getBeds(req: Request, res: Response) {
        try {
            ...
            const data = await this.db
                .collection(COLLECTIONS.BEDS)
                .findOne({ hospitalId: new ObjectId(id) });
